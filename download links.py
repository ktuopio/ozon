...
if not os.path.exists(r"C:\foldername"):
    os.makedirs(r"C:\foldername")

# Скачивание и сохранение файлов
for link, name in zip(pdf_links, pdf_names):
    response = requests.get(link, headers=headers, verify=False)
    file_path = os.path.join(r"C:\foldername", f"{name}.pdf")

    # Проверка, существует ли уже файл с таким именем, и создание копии при необходимости
    counter = 1
    while os.path.exists(file_path):
        file_path = os.path.join(r"C:\foldername", f"{name}_{counter}.pdf")
        counter += 1
